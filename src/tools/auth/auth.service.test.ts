...
describe('AuthService', () => {
    ...
    test('login - can login success', async () => {
        const mockResponse = {
            status: 302,
            headers: {
                location: "https://qldt.hanu.edu.vn/?CurrUser=eyJJRFVzZXIiOi03ODI3MzMyMTM3OTE&gopage="
            }
        };
        ...
        expect(httpClient.defaults.headers.Authorization).toBe('Bearer eyJJRFVzZXIiOi03ODI3MzMyMTM3OTE');
    });
    ...
    test('login - can login failed', async () => {
        ...
        await expect(AuthService.login("fail", "fail")).rejects.toThrow("Login failed");
    });
});
